...
import { simulateMouseEvent } from 'neurons-dom';
import { bind } from '../../src/binding';
import { Binding, Property, Emitter } from '../../src/binding/factory/decorator';
import { IEmitter } from 'neurons-emitter';

describe('Neurons::Binding', () => {
    beforeAll(() => {
    ...
    describe('template compile & instantiate & renderer & binding.', () => {
        describe('create with plain text without a container', () => {
            ...
            const elementRef = bind(template);
        ...
        describe('create with plain text without a valided container', () => {
            ...
            const template = `plain text`;
            const elementRef = bind(template, { container: dom });
        ...
        describe('binding class: [class]="className"', () => {
            ...
            const elementRef = bind(template, { container: dom, state: { className: 'binded-class' } });
        ...
        describe('binding style: [style]="stylestring"', () => {
            ...
            const elementRef = bind(template, { container: dom, state: { stylestring: 'opacity: 1' } });
        ...
        describe(`binding event: (click)="onClick(1, $event)"`, () => {
            ...
            const elementRef = bind(template, { container: dom, state: state });
    ...
    describe('Binding decoritor & instantiater & bootstrap.', () => {
        describe('decorate a "plain text" Binding', () => {
            @Binding({ selector: 'plain-text', template: 'plain text' })
            class TestUIBinding { }
            const dom = document.createElement('div');
            const elementRef = bind(`<plain-text/>`, { container: dom });
            it('dom.innerHTML should be "plain text"', () => {
                expect(dom.innerHTML).toBe('plain text');
            });
        });
        describe('decorate a "plain text" Binding via binding varible text = "plain text"', () => {
            @Binding({ selector: 'plain-text-binding', template: '{{text}}' })
            class TestUIBinding {
                @Property() text = 'plain text'
            }
            const dom = document.createElement('div');
            const elementRef = bind(`<plain-text-binding/>`, { container: dom });
            it('dom.innerHTML should be "plain text"', () => {
                expect(dom.innerHTML).toBe('plain text');
            });
        });
        describe('decorate a simple Binding via binding varibles.', () => {
            @Binding({ selector: 'simple-binding', template: '<div [class]="className" [style.opacity]="opacity">{{text}}</div>' })
            class TestUIBinding {
            ...
            const dom = document.createElement('div');
            const elementRef = bind(`<simple-binding/>`, { container: dom });
        ...
        describe('multi different Binding in template.', () => {
            @Binding({ selector: 'ui-a', template: '{{text}}' })
            class TestUIBindingA {
                @Property() text = 'plain text';
            }
            @Binding({ selector: 'ui-b', template: '<div [class]="className" [style.opacity]="opacity">{{text}}</div>' })
            ...
            const dom = document.createElement('div');
            const elementRef = bind(`<ui-a/><ui-b/>`, { container: dom });
        ...
        describe('decorate a Property text, and change value to "changed text" via binding plain value.', () => {
            @Binding({ selector: 'property-binding', template: '<div [class]="className" [style.opacity]="opacity">{{text}}</div>' })
            class TestUIBinding {
                @Property() className = 'test-class';
                @Property() text = 'plain text';
                @Property() opacity = 1;
            ...
            const dom = document.createElement('div');
            const elementRef = bind(`<property-binding [text]="'changed text'"/>`, { container: dom });
        ...
        describe('decorate a properties, and change value via setState" via binding text varible.', () => {
            @Binding({ selector: 'property-binding-varible', template: '<div [class]="className" [style.opacity]="opacity">{{text}}</div>' })
            class TestUIBinding {
                @Property() className = 'test-class';
                @Property() text = 'plain text';
                @Property() opacity = 1;
            ...
            const dom = document.createElement('div');
            const elementRef = bind(`<property-binding-varible [text]="text" [className]="className" [opacity]="opacity"/>`, {
                container: dom, state: {
                    text: 'changed text',
                    className: '',
                    opacity: 1
        ...
        describe('decorate a emitters, and listen value change via binding.', () => {
            @Binding({ selector: 'emitter-binding-varible', template: '<div (click)="onClick($event)" [class]="className" [style.opacity]="opacity">{{text}}</div>' })
            class TestUIBinding {
                @Property() className = 'test-class';
                @Property() opacity = 1;
                @Property() text = 'plain text';
            ...
            const elementRef = bind(`<emitter-binding-varible (itemClicked)="onItemClicked($event)"/>`, { container: dom, state: state });
            simulateMouseEvent(dom.children.item(0), 'click');
        ...
        describe('repeat Binding via *for', () => {
            @Binding({ selector: 'repeat-binding', template: '<div>{{text}}</div>' })
            class TestUIBinding {
                @Property() text = 'plain text';
            }
            const dom = document.createElement('div');
            const elementRef = bind(`<repeat-binding *for="text in array" [text]="text"/>`, {
                container: dom, state: {
        ...
        describe('inset content into Binding via inner content placeholder', () => {
            @Binding({ selector: 'outlet-content-binding', template: '<div><content/></div>' })
            class TestUIBinding {
                @Property() name = 'content-test'
            }
            const dom = document.createElement('div');
            const elementRef = bind(`<outlet-content-binding>outlet text</outlet-content-binding>`, {
                container: dom, state: {}
            });
            it('dom.children.item(0).innerHTML should be "outlet text"', () => {
                console.log(dom);
                expect(dom.children.item(0).innerHTML).toBe('outlet text');
            });
        });
    });
