...
export class XMLASTSelector {
    constructor(private nodeOrNodes: IXMLASTNode | IXMLASTNode[]) {
        this.nodes = (isArray(nodeOrNodes) ? nodeOrNodes : [nodeOrNodes]) as IXMLASTNode[];
    }
    nodes: IXMLASTNode[] = [];
    text(): string[] {
        return this.nodes.reduce((ret, node) => {
            if (node && node.childNodes) {
                ret = ret.concat(node.childNodes.filter(n => n.type === XMLASTNodeType.TEXT).map(n => n.content))
            }
            return ret;
        }, []);
    }
    each(fn: (node: IXMLASTNode, index: number) => void) {
        return this.nodes.forEach(fn);
    }
    select(tagName: string): XMLASTSelector {
        const result = this.nodes.reduce((ret, node) => {
            if (node && node.childNodes) {
                ret = ret.concat(node.childNodes.filter(n => n.name === tagName))
            }
            return ret;
        }, []);
        return new XMLASTSelector(result);
    }
    get(index): IXMLASTNode {
        return this.nodes[index];
    }
}

export function selectXMLAST(nodeOrNodes: IXMLASTNode | IXMLASTNode[]): XMLASTSelector {
    return new XMLASTSelector(nodeOrNodes);
}
