import { INeElement, INeBindingScope, INeElementBinding, IBindingMetadata, EventName, INeBindingRef, IBindingRefFactory } from '../common/interfaces';
...
import { getUIBindingMetadata, createUIBindingInstance } from '../factory/binding';
...
export class NeElement implements INeElement {
    constructor(
        selector: string,
        ...
        // metadata
        this._metadata = getUIBindingMetadata(selector);
        // 采集input output
        if (this._metadata) {
    ...
    private _metadata: IBindingMetadata;
    ...
    protected initialize() {
        ...
        if (!this.placeholder.parentNode || !this._bindingRef) return;
        this.onInit();
        // 标记初始化完成
        this.inited = true;
        this.attached = true;
...
registerBindingElement(BindingElementTypes.ELEMENT, NeElement);