...
import { StateEntries, IElementRef } from '../../compiler/common/interfaces';
...
`)
export class PopupOverlayRef<T extends StateEntries> implements IPopupOverlayRef<T> {
    ...
    private _ref: IElementRef<IPopupOverlayOption<T>>;
    ...
    appear() {
        ...
        this._ref.setState({ animationEnter: true });
        this._cancelAnimation = Animation.start({
            duration: 180,
            onEnter: () => {
                this._ref.setState({ animationDone: true });
    ...
    disappear() {
        ...
        this._cancelAnimation = Animation.start({
            duration: 120,
            onEnter: () => {
                this._ref.setState({ animationDone: false });
                this.onDispear.emit();
            },
            onDone: () => {
