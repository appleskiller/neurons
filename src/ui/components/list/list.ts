import { UIBinding, Property, Element, Inject, Emitter } from '../../factory/decorator';
import { IEmitter } from '../../../helper/emitter';
import { StateChanges, IChangeDetector } from '../../compiler/common/interfaces';
import { isDefined, isDate, findAValidValue, ObjectAccessor } from '../../../utils';
...
import { getMaxHeight, createElement, removeMe } from '../../../utils/domutils';
        ...
        <div #container [class]="{'ne-list': true, 'enable-selection': enableSelection}" (scroll)="onScroll($event)">
            <div #shim class="ne-list-shim" [style.height]="contentHeight">
                <div #content class="ne-list-content" [style.top]="offset">
                    ...
                    >
                        <ne-binding
                            *bind="{'hostBinding': itemRendererBinding, 'source': itemRenderer}"
...
export class List<T> {
    ...
    onChanges(changes: StateChanges) {
        ...
        if (changes && ('dataProvider' in changes || 'active' in changes || this._isInvalidTypicalHeight())) {
            this._resetDataProvider();
        }
    }
    onResize() {
    ...
    protected sliceDataProvider(dataProvider, startIndex, endIndex) {
        // if (startIndex === 0 && endIndex === dataProvider.length - 1) {
        //     this.nativeDataProvider = this.dataProvider;
        // } else {
    ...
    private _resetNativeDataProvider() {
        ...
        this.contentHeight = this._typicalHeight * this.dataProvider.length;
        if (this.contentHeight <= 0) {
            ...
            this.offset = 0;
            this.startIndex = 0;
            this.endIndex = this.dataProvider.length - 1;
        } else {
            ...
            this.endIndex = Math.min(this.dataProvider.length - 1, scrollIndex + itemsPerCol);
            this.startIndex = this.endIndex - Math.min(itemsPerCol, this.dataProvider.length - 1);
            this.offset = this.startIndex * this._typicalHeight;
        }
        this.sliceDataProvider(this.dataProvider, this.startIndex, this.endIndex);
    }
